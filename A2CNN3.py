...
class Flatten(nn.Module):
    def forward(self, x):
        return x.reshape(x.size(0), -1)#x.view(x.size(0), -1)
...
class CNNBase(nn.Module):
    ...
    def evaluate_actions(self, input, actions, device):
        action_probs, vals = self.forward(input)
        #print(action_probs.size())
        dist = self.dist(action_probs).to(device=device)
        #print(dist)
        #print(actions.size())
        #dist = self.dist(action_probs)
        #print(actions)
        #logs = torch.gather(dist,1, actions)
        action_logs = dist.log_probs(actions).to(device=device)
